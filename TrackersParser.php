...
class TrackerParser{
	protected $login;
	protected $password;
	protected static $cookiedir = 'cookies';
	...
	protected function curl($url,$post=false,$cookiefile=false,$cookies = false,$headers = false){
		...
		if($cookiefile){
			curl_setopt($curl, CURLOPT_COOKIEJAR, self::$cookiedir.'/'.$cookiefile);
			curl_setopt($curl, CURLOPT_COOKIEFILE, self::$cookiedir.'/'.$cookiefile);
		}
		if($cookies){
